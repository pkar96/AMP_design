...
######################################################
# CHECKS IF RESIDUE CONTRIBUTES TO STRUCTURE OR NOT  #
######################################################

def return_colour_DB(key, data):
   check=0
...
def helical_wheel(IP_graph, DB_graph, data, protein_name, output_directory, IP_database):
 
   XY_data_DB=[]
   XY_data_IP=[]
   DB_graph_keys=DB_graph.keys()
   for i in range(0,len(DB_graph_keys)):
      theta=(i*106.0/360.0)*2*math.pi
      X_val_DB=  (math.cos(theta))
      Y_val_DB=  (math.sin(theta))
      XY_data_DB.append([X_val_DB,Y_val_DB])
   IP_graph_keys=IP_graph.keys()
   for i in range(0,len(IP_graph_keys)):
      theta=(i*106.0/360.0)*2*math.pi
      X_val_IP=  (math.cos(theta))
      Y_val_IP=  (math.sin(theta))
      XY_data_IP.append([X_val_IP,Y_val_IP])  
   ...
   for i in range(99,901):
        ...
        draw.point((1901,i), fill=color)
        draw.point((1902,i), fill=color)
        draw.point((1000,i), fill=color)

   #calculate axes range:
   X_min_DB = float("inf")
   X_max_DB = float("inf")*-1
   Y_min_DB = float("inf")
   Y_max_DB = float("inf")*-1

   X_min_IP = float("inf")
   X_max_IP = float("inf")*-1
   Y_min_IP = float("inf")
   Y_max_IP = float("inf")*-1

   for i in range(0, len(XY_data_DB)):
        if XY_data_DB[i][0] > X_max_DB:
            X_max_DB = float(XY_data_DB[i][0])
        if XY_data_DB[i][0] < X_min_DB:
            X_min_DB = float(XY_data_DB[i][0])
        if XY_data_DB[i][1] > Y_max_DB:
            Y_max_DB = float(XY_data_DB[i][1])
        if XY_data_DB[i][1] < Y_min_DB:
            Y_min_DB = float(XY_data_DB[i][1])
   for i in range(0, len(XY_data_IP)):
        if XY_data_IP[i][0] > X_max_IP:
            X_max_IP = float(XY_data_IP[i][0])
        if XY_data_IP[i][0] < X_min_IP:
            X_min_IP = float(XY_data_IP[i][0])
        if XY_data_IP[i][1] > Y_max_IP:
            Y_max_IP = float(XY_data_IP[i][1])
        if XY_data_IP[i][1] < Y_min_IP:
            Y_min_IP = float(XY_data_IP[i][1])
    
   #plot DB_helical_wheel:
   r=15
   font = ImageFont.truetype(IP_database+"/formatting/arial.ttf", 15)
   for i in range(0, len(XY_data_DB)):
        Y_point = (((XY_data_DB[i][0]-X_min_DB)/(X_max_DB-X_min_DB)*700)+200)
        X_point = 1000-(((XY_data_DB[i][1]-Y_min_DB)/(Y_max_DB-Y_min_DB)*800)+100)
        colour= return_colour_DB(DB_graph_keys[i], data)
        draw.ellipse((Y_point-r,  X_point-r , Y_point+r, X_point+r), fill=colour )
        draw.text((Y_point+r,X_point+r),str(DB_graph_keys[i]),fill=(0,0,0),font=font)
        draw.text((Y_point+2*r,X_point+2*r),str(DB_graph[DB_graph_keys[i]]['name']),fill=(0,0,0),font=font)

   #plot IP_helical_wheel
   for i in range(0, len(XY_data_IP)):
        Y_point = (((XY_data_IP[i][0]-X_min_IP)/(X_max_IP-X_min_IP)*700)+1100)
        X_point = 1000-(((XY_data_IP[i][1]-Y_min_IP)/(Y_max_IP-Y_min_IP)*800)+100)
        colour= return_colour_IP(IP_graph_keys[i], data)
        draw.ellipse((Y_point-r,  X_point-r , Y_point+r, X_point+r), fill=colour )
        draw.text((Y_point+r,X_point+r),str(IP_graph_keys[i]),fill=(0,0,0),font=font)
        draw.text((Y_point+2*r,X_point+2*r),str(IP_graph[IP_graph_keys[i]]['name']),fill=(0,0,0),font=font)

   #output image:
   img.save(output_directory+'/'+protein_name, "png")
...
def subgraph(DB_graph, IP_graph):
...
###########################################
#  COUNT THE NUMBER OF LYSIDNE RESIDUES   #
###########################################

def count_freq_lys(sequence):
   freq=0
...
def simulated_anneal(IP_graph, graph_list, cycles, cooling_schedule, bijection_threshold, probability_flag, design_data, verbose_flag):
    ...
    for i in range(0,cycles):
        ...
        temperature = cooling_scheduler(i, cycles, cooling_schedule, 1, float(1/cycles))
        ...
        new_residue=random.choice(design_data[chosen_residue])
        IP_graph[chosen_residue]['name'] = new_residue

        #score mutant:
        NEW_score = SA_scorer(IP_graph, graph_list, bijection_threshold)
        if new_residue== 'K' or new_residue=='R':
            sequence=[]
            for j in range(0,len(IP_graph_keys)):
                  sequence.append(IP_graph[IP_graph_keys[j]]['name'])
            freq_lys=count_freq_lys(sequence)
            freq_arg=count_freq_arg(sequence)
            if freq_lys>4:
                  NEW_score=NEW_score+10*(freq_lys-4)
            if freq_arg> int(12.0/100.0*20.0):
                  NEW_score=NEW_score+10*(freq_arg-2)
    ...
    #END function:
    return [BEST_sequence, BEST_score, score_log]
...
if len(sys.argv) < 7 or len(sys.argv) > 21:
...
#definitions:
opts, args = getopt.getopt(sys.argv[1:], "i:o:d:n:a:s:b:p:v:S:", ["input=", "output=", "database=", "iterations=", "annealer=", "seed=", "bijection=", "probability=","verbose=", "structure="])
...
[BEST_sequence, BEST_score, score_log] = simulated_anneal(IP_graph, graph_list, cycles, cooling_schedule, bijection_threshold, probability_flag, design_data, verbose_flag)
...
'''
print AM_helices[0]
helical_wheel_database=subgraph(IP_graph, graph_list[0])
helical_wheel(IP_graph, graph_list[0], helical_wheel_database, AM_helices[0][0:4], str(output_directory))
    
'''
#output common_graphs.png:
helical_wheel_database=[]
for i in range(0, len(AM_helices)):
    helical_wheel_database=subgraph(IP_graph, graph_list[i])
    helical_wheel(IP_graph, graph_list[i], helical_wheel_database, AM_helices[i][0:4], str(output_directory), str(database))
